...
public class EinstellungenViewController implements Initializable
{
	...
	public void initialize(URL location, ResourceBundle resources)
	{
		...
		{
			@Override public String toString(Studiengang object)
			{
				return object.getName();
			}

			@Override public Studiengang fromString(String string)
			{
				return null;
			}
		});
		...
		semesterChoicebox.promptTextProperty().bind(Bindings.when(studiengangChoicebox.getSelectionModel().selectedItemProperty().isNull()).then("Bitte erst Studiengang wählen").otherwise("Studiensemester wählen"));
		semesterChoicebox.setConverter(new StringConverter<Studiensemester>()
		{
			@Override public String toString(Studiensemester object)
			{
				return object.getName();
			}

			@Override public Studiensemester fromString(String string)
			{
				return null;
			}
		});
		...
		darkmodeCheckBox.textProperty().bind(Bindings.when(darkmodeCheckBox.selectedProperty()).then("aktiv").otherwise("inaktiv"));
	...
	{
		List<List<Studiensemester>> studiensemester=SchreiberLeser.getDropdownMenue().getStudiengaenge().stream().filter((item)->item.equals(studiengang)).map(Studiengang::getStudiensemester).collect(Collectors.toList());

		return FXCollections.observableList(studiensemester.stream().flatMap(Collection::stream).collect(Collectors.toList()));
	}
}
